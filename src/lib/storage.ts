...
import type { Customer, Product, Quotation, CompanyDetails, User, ID, Supplier, Order, StockMovement, SupplierPrice, PurchaseOrder, AutomationRule, AIInsight, DemandForecast } from './types';
...
// Company
export const getCompany = (): CompanyDetails => get<CompanyDetails>(KEY.company, { name: 'Your Company' });
...
export const loginLocal = (email: string, password: string, role: User['role']) => {
  ...
  if (!existing) {
    ...
    if (existing.role !== role) {
      upsertUser({ email, password: existing.password, role });
    }
  }
...
export const currentSession = (): User | null => {
  const session = get(KEY.session, null);
...
export const createAIInsight = (insight: Omit<AIInsight, 'id' | 'createdAt'>): AIInsight => {
  const all = listAIInsights();
  const next: AIInsight = { 
    ...insight, 
    id: uuid(), 
    createdAt: new Date().toISOString()
  };
  set(KEY.aiInsights, [next, ...all]);
  return next;
};
export const updateAIInsight = (id: ID, patch: Partial<AIInsight>): AIInsight | null => {
  const all = listAIInsights();
  const idx = all.findIndex(insight => insight.id === id);
  if (idx === -1) return null;
  const updated = { ...all[idx], ...patch } as AIInsight;
  all[idx] = updated;
  set(KEY.aiInsights, all);
  return updated;
};
...
// AI-Powered Features (Mock implementations for future integration)
...
export const generateAIInsights = (): AIInsight[] => {
  ...
  const highMarginProducts = products.filter(p => {
    const analysis = calculateProfitAnalysis(p.id, p.sellingPrice);
    return analysis && analysis.profitMargin > 40;
  });
  ...
  if (highMarginProducts.length > 0) {
    insights.push(createAIInsight({
      ...
      description: `${highMarginProducts.length} products have margins above 40%. Consider competitive pricing analysis.`,
      confidence: 85,
      impact: 'medium',
      data: { products: highMarginProducts.map(p => p.id) },
      actionable: true
    }));
  }
  ...
  const slowMovingProducts = products.filter(p => {
    const productOrders = orders.flatMap(o => o.items.filter(i => i.productId === p.id));
    const totalSold = productOrders.reduce((sum, item) => sum + item.quantity, 0);
    return p.stock > 0 && totalSold === 0;
  });
  ...
  if (slowMovingProducts.length > 0) {
    insights.push(createAIInsight({
      ...
      description: `${slowMovingProducts.length} products have no recent sales. Consider promotions or discontinuation.`,
      confidence: 90,
      impact: 'high',
      data: { products: slowMovingProducts.map(p => p.id) },
      actionable: true
    }));
  }
  ...
  if (topSuppliers.length > 0) {
    insights.push(createAIInsight({
      ...
      description: `Focus on ${topSuppliers.length} high-rated suppliers for better reliability and pricing.`,
      confidence: 80,
      impact: 'medium',
      data: { suppliers: topSuppliers.map(s => s.id) },
      actionable: true
    }));
  }
