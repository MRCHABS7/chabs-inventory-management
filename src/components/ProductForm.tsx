            ...
            <div className="bg-green-50 border border-green-200 rounded-lg p-4">
              ...
              <div className="grid grid-cols-3 gap-4 text-sm">
                <div>
                  ...
                  <div className="font-bold text-green-600">${(draft.sellingPrice - draft.costPrice).toFixed(2)}</div>
                </div>
                <div>
          ...
          {bomItems.length > 0 && (
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
              ...
              <div className="text-sm">
                ...
                <div>Total BOM Cost: ${bomItems.reduce((sum, item) => sum + (item.costPrice * item.quantity), 0).toFixed(2)}</div>
              </div>
            </div>
          )}
        </div>
          ...
          {supplierPrices.map((item, index) => (
            <div key={index} className="grid md:grid-cols-5 gap-3 p-3 bg-gray-50 rounded-lg">
              ...
              <input 
                ...
                onChange={(e) => updateSupplierPrice(index, 'minimumQuantity', Number(e.target.value))} 
              />
              <input 
                className="input" 
                type="number" 
                ...
                value={item.leadTime} 
                onChange={(e) => updateSupplierPrice(index, 'leadTime', Number(e.target.value))} 
              />
              <button 
                type="button" 
